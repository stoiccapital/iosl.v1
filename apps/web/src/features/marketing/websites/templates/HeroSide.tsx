...
function HeroSection({
  ...
  return (
    <section className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-6 pb-16 pt-6 md:grid-cols-[1.1fr_1fr]">
      <div>
        <h1 className="text-4xl font-semibold leading-[1.05] tracking-tight sm:text-6xl">
          {headline}
        </h1>
        <p className="mt-6 max-w-lg text-lg text-neutral-600">{sub}</p>
      </div>
...
export function HeroSideRidePage(props: TemplateProps) {
  ...
  return (
    <Shell {...props}>
      ...
      />
      <section id={RIDE_FORM_ID} className="mx-auto max-w-6xl px-6">
...
export function HeroSideDrivePage(props: TemplateProps) {
  const { site, locale } = props;
  const c = site.content;
  ...
  return (
    <Shell {...props}>
      <HeroSection
        ...
        imageUrl={c.hero.imageUrl}
      />
      <section id={DRIVE_FORM_ID} className="mx-auto max-w-6xl px-6">
        <div className="max-w-3xl">
          <h2 className="mb-4 text-lg font-semibold">{cta}</h2>
