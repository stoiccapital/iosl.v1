...
type Props = {
  ...
  ctaLabel: string;
  className?: string;
  calcContext?: RideCalcContext | null;
};
...
export function LeadForm({ slug, locale, theme, ctaLabel, className, calcContext }: Props) {
  const submit = useSubmitToSite(slug);
  const [values, setValues] = useState({ name: '', email: '', phone: '', city: '', note: '' });
    ...
    try {
      await submit.mutateAsync({
        ...
        payload: {
          ...
          ...(calcContext
            ? {
                startCityId: calcContext.startCityId,
                endCityId: calcContext.endCityId,
                distanceKm: calcContext.distanceKm,
                estimateCents: calcContext.estimateCents,
              }
            : {}),
        },
      });
      setDone(true);
      setValues({ name: '', email: '', phone: '', city: '', note: '' });
    } catch {
  ...
  if (done) {
    return (
      <p className={cn('text-sm', successTheme[theme], className)}>
        {t(UI_COPY.ride.success, locale)}
      </p>
    );
  }

  const c = UI_COPY.ride;
  return (
